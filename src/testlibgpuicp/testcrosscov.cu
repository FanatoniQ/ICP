...
int main(int argc, char **argv)
{
    ...
    float *h_ref_cross_covs = get_cross_covs_cpu(P, Plines, Pcols, Q, Qlines, Qcols, d_dist, dist_0, dist_1, dist_pitch);
    float *h_r = (float*)malloc(Rlines * Rcols * sizeof(float));
    cudaMemcpy2D(h_r, Rcols * sizeof(float), d_R, r_pitch, Rcols * sizeof(float), Rlines, cudaMemcpyDeviceToHost);
    cudaCheckError();
