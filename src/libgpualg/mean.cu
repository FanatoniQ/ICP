...
// computes line sum
__device__ __host__ float get_line_sum(const float *line, int nbvals)
{
    int j;
    float sum = 0;
    for (j = 0; j < nbvals; ++j)
...
// computes line mean
__device__ __host__ float get_line_mean(const float *line, int nbvals)
{
    return get_line_sum(line, nbvals) / nbvals;
}

// computes column sum (pitch is NOT in bytes)
__device__ __host__ float get_column_sum(const float *A, int colidx, int nblines, int pitch)
{
    int i;
    float sum = 0;
    for (i = 0; i < nblines; ++i)
...
__device__ __host__ float get_column_mean(const float *A, int colidx, int nblines, int pitch)
{
...
__global__ void dumb_mean_kernel_0(const float *d_A, float *d_sumA, int pitch, int width, int height)
{
    int idx = threadIdx.x;
    if (idx >= width)
        return;
...
{
    extern __shared__ float s_data[]; // s_data is of size blockDim.x
    ...
    float *d_sumAline = (float *)((char *)d_sumA + lineid * reducepitch);
    if (threadid == 0)
...
__global__ void tree_reduce_sum_kernel_0(const float *d_A, float *d_sumA, int pitch, int width, int height, int reducepitch)
{
    ...
    //printf("Reduce Index: %d\n", blockIdx.x + reducepitch * lineid);
    float *d_sumAline = (float *)((char *)d_sumA + blockIdx.y * reducepitch);
    if (threadid == 0)
        d_sumAline[blockIdx.x] = s_data[0];
    //d_sumA[blockIdx.x + reducepitch * lineid] = s_data[0]; // we store at pos x,y the partial mean
    //d_sumA[blockIdx.x + (height / blockDim.x) * lineid] = s_data[0]; // we store at pos x,y the partial mean
...
__global__ void tree_reduce_mean_kernel_0(const float *d_A, float *d_sumA, int pitch, int width, int height, int reducepitch, size_t denom)
{
    extern __shared__ float s_data[]; // s_data is of size blockDim.y
    int threadid = threadIdx.x; // thread id in the block
    int lineid = blockIdx.y * blockDim.x + threadIdx.x; // line
...
__host__ void reduce_0(enum MatrixReduceOP op, float *d_A, float **d_sum, size_t width, size_t height, size_t pitch, size_t *reducepitch, int threads)
{
    ...
    if (*d_sum == nullptr)
    {
        ...
        cudaCheckError();
	**/
        *reducepitch = width * sizeof(float);
    ...
    {
        tree_reduce_sum_kernel_0<<<blocks, threads, threads * sizeof(float)>>>(d_A, *d_sum, pitch, width, height, *reducepitch);
    } else {
        if (op == MatrixReduceOP::MEAN)
            tree_reduce_mean_kernel_0<<<blocks, threads, threads * sizeof(float)>>>(d_A, *d_sum, pitch, width, height, *reducepitch, height);
    ...
    if (nbblocksPerColumn > 1)
    {
        ...
        std::cerr << "nbthreads: " << threads << " nbcolumns: " << blocks.x << " nbblocksPerColumns: " << blocks.y << std::endl;
        if (op == MatrixReduceOP::MEAN)
            tree_reduce_mean_kernel_0<<<blocks, threads, threads * sizeof(float)>>>(*d_sum, *d_sum, pitch, width, nbblocksPerColumn, *reducepitch, height);
        else if (op == MatrixReduceOP::SUM)
            tree_reduce_sum_kernel_0<<<blocks, threads, threads * sizeof(float)>>>(*d_sum, *d_sum, pitch, width, nbblocksPerColumn, *reducepitch);
        cudaDeviceSynchronize();
        cudaCheckError();
    }
}
